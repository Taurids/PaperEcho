...
def squared_norm_jacobian(y, x):
    D_inter, interpolates = y, x
    grad = torch.autograd.grad(D_inter, interpolates, grad_outputs=torch.ones_like(D_inter), create_graph=True)[0]
    norm_gradients = torch.norm(grad, p='fro', dim=[2, 3])  # [bs, channel]
    norm2_jac = torch.sum(torch.square(norm_gradients), dim=-1)
    return norm2_jac


def calculate_scaled(y, x, scaling_coeff=10):
    norm2_jac = squared_norm_jacobian(y, x)
    norm2_jac = torch.mean(norm2_jac)
    norm_discriminator = torch.mean(torch.square(y))
    scale = 1. / torch.sqrt(scaling_coeff * (norm2_jac + norm_discriminator) + 1.)
    return scale
