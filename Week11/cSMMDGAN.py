...
# Loss function
mmd_loss = Echo2Torch.mmd.MMDLoss(kernel_type='rq').to(device)  # 0.25, 0.5, 1, 2, 4
...
for epoch in range(args.n_epochs):
    for i, (img, label) in enumerate(train_loader):
        ...
        scale = Echo2Torch.mmd.calculate_scaled(Enc_X_D, real_img, args.scaling_coeff)
        loss_D = -torch.sqrt(mmd_errD) * scale
        ...
        if (i + 1) % args.n_critic == 0:
            ...
            mmd_errG = mmd_loss(net_D(real_img, label), net_D(fake_img, label))
            loss_G = torch.sqrt(mmd_errG)
