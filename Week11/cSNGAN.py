...
ds_instance = torchvision.datasets.MNIST(args.dir_dataset, train=True, download=True, transform=ds_transform)
...
net_G = mmdgan.Generator(args).to(device).train()
net_D = mmdgan.Discriminator(args).to(device).train()
...
for epoch in range(args.n_epochs):
    for i, (img, label) in enumerate(train_loader):
        ...
        hinge_lossD = mmdgan.hinge_loss_dis(Enc_Y_D, Enc_X_D)
        loss_D = hinge_lossD
        ...
        if (i + 1) % args.n_critic == 0:
            ...
            hinge_lossG = mmdgan.hinge_loss_gen(Enc_Y_D)
            loss_G = hinge_lossG
        ...
        if batches_done % args.sample_interval == 0:
            ...
            torch.save(optimizer_G.state_dict(), os.path.join(output_dir, 'optimG.pth'))
            torch.save(optimizer_D.state_dict(), os.path.join(output_dir, 'optimD.pth'))
            torch.save(scheduler_G.state_dict(), os.path.join(output_dir, 'schedG.pth'))
            torch.save(scheduler_D.state_dict(), os.path.join(output_dir, 'schedD.pth'))
    # decay LR
    scheduler_G.step()
    scheduler_D.step()
