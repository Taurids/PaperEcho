...
# Categorical to int
for i in range(61):
    df_new[i] = LabelEncoder().fit_transform(df_new[i])  # 字符特征硬编码
...
err_rate = KRR_l2_unif_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-9, -3])
...
err_rate = KRR_l2_align_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-9, -3])
print('err rate in the validation is: {}'.format(err_rate))
...
err_rate = KRR_l2_alignf_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-9, -3])
print('err rate in the validation is: {}'.format(err_rate))
...
err_rate = KRR_l2_1_stage_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-9, -3])
print('err rate in the validation is: {}'.format(err_rate))
