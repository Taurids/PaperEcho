...
for i in [0, 2, 3, 5, 6, 8, 9, 11, 13, 14, 16, 18, 19]:
    df[i] = LabelEncoder().fit_transform(df[i])  # 字符特征硬编码
# str to int
...
err_rate = KRR_l2_unif_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-4, 3])
...
err_rate = KRR_l2_align_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-4, 3])
print('err rate in the validation is: {}'.format(err_rate))
...
err_rate = KRR_l2_alignf_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-4, 3])
print('err rate in the validation is: {}'.format(err_rate))
...
err_rate = KRR_l2_1_stage_predict(X_train, X_valid, y_train, y_valid, p_base_kernel=[-4, 3])
print('err rate in the validation is: {}'.format(err_rate))
